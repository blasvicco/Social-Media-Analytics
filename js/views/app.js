/*global define*/
define([
    'jquery', 'underscore', 'backbone', 'collections/todos', 'views/todos', 'text!templates/stats.html', 'common'
    ...
    loadPage : function() {
      ...
      if (this.searchBy) {
        ...
        collection = Todos.filter(function(model) {
          return _.some([
              model.get('provider'), model.get('actor_name'), model.get('actor_username'), model.get('actor_description'),
              model.get('title'), model.get('activity_date')
          ], function(value) {
            var exp = q + '.*';
      ...
      var items = collection.slice(this.page * this.step, (this.page + 1) * this.step);
      ...
      if (items.length < this.step) this.loadMore();
      this.page++;
      this.$('#loaded-items').html(((this.page - 1) * this.step) + items.length);
    },
